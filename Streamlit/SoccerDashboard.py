...
def createBarChart(data, x_axis, y_axis, title, text, color='blue'):
...
def createLinePlot(data, x_axis, y_axis, title, hover=None):
    ...
    # Cria um gráfico de linha com Plotly Express
    fig = px.line(df, x=x_axis, y=y_axis, hover_data=hover,
                  title=title
                  )
